...
2. Hand the user this preview link and the controls:
   ...
   **Open:** `https://imraiy-tv.myshopify.com/products/voltical-core-5k-10k?preview_theme_id=193289027910&edit=1`
   ...
   - **Align:** snaps small accidental drift (up to 14px) on the selected element back to its perfect position — or, if nothing is selected, across every tile, image, icon and text box at once. Only a tile's own position snaps on both axes (back to its grid spot) and only a tile's own size resets to regular — everything a tile contains (images, icons, text) is positioned vertically by its own CSS on purpose, so Align never touches dy for anything but a tile itself. Horizontally, text is the exception: it always snaps to dead-center no matter how far off it drifted, while an image/icon's horizontal position only snaps back when the drift is small, same as a tile. Shift+click Align for a bigger correction (up to 40px, tiles/images/icons only). Deliberate larger moves on those are left alone, so this is safe to click any time something looks slightly off after dragging.
