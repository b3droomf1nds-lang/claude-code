...
2. Hand the user this preview link and the controls:
   ...
   **Open:** `https://imraiy-tv.myshopify.com/products/voltical-core-5k-10k?preview_theme_id=193289027910&edit=1`
   ...
   - **Align:** snaps small accidental drift (up to 14px) on the selected element back to its perfect position — or, if nothing is selected, across every tile, image, icon and text box at once: tiles return to the grid, images/icons re-center on both axes relative to their tile, and anything slightly resized returns to its regular size. Text boxes only get straightened left/right — their vertical position is left alone, since several are deliberately pinned to a spot (e.g. the bottom of an image). Shift+click Align for a bigger correction (up to 40px). Deliberate larger moves are left alone, so this is safe to click any time something looks slightly off after dragging.
