...
2. Hand the user this preview link and the controls:
   ...
   **Open:** `https://imraiy-tv.myshopify.com/products/voltical-core-5k-10k?preview_theme_id=193289027910&edit=1`
   ...
   - **Align:** snaps small accidental drift (up to 14px, or 40px with Shift+click) on a tile back to its perfect grid position, and a text box's (caption/heading, or the big live-computed stat number like "67h") horizontal position — or, if nothing is selected, straightens every tile and text box at once. Deliberate larger moves are left alone. Tiles snap to whichever of the 6 grid slots they're now CLOSEST to (not just back to their own original slot) — so to swap two tiles, drag each roughly into the other's spot and click Align to lock both exactly into place; this works because a tile's real grid cell never actually moves when dragged, only its on-screen transform does, so all 6 real slots are always known. A text box (or stat number) is only ever straightened left/right by Align — never moved vertically or resized, since its vertical spot is set by its own CSS on purpose. Images and icons are left alone entirely by Align — once placed, they stay exactly where put, so a deliberate small nudge on a photo never gets silently reverted.
   - **Typing:** double-click a caption/heading (`.pdp-cmp__eyebrow`, `.pdp-cmp__sub`, `.pdp-cmp__select-heading` — anything that's literal text, not one of the live-computed numbers) to drill into it, same as any image/icon — it's still draggable with a single click-hold from there. Once it's selected, either press **Enter** or just click it again (a plain click that doesn't turn into a drag) to start typing into it in its own font; the cursor lands at the end of the existing text, so it's additive. Press **Enter** again to start a new line lower in the same box. Click elsewhere or press **Esc** to stop typing and save. (Drag and type stay separate gestures: the double-click that first drills into a caption always just selects it, and click-to-type only fires on a *second* click once it's already selected — so dragging a caption right after double-clicking it never gets hijacked into typing mode.)
   - **Delete text:** with a text box selected (not currently being typed into), press **Backspace** or **Delete** to clear it out — same idea as pressing Enter to type, just the reverse.
   - **Undo** button in the toolbar steps back one action at a time — a move, a resize, typing/deleting a caption, or a whole Align/Reset click. Holding an arrow key or scrolling to resize counts as one action, not one per tick.
