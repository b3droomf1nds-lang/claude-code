...
description: >-
  ...
  the image/text/icon inside, drag or scroll to resize, arrows to nudge. Use
  this skill whenever the user types /canva, says "canva mode", asks to move,
...
Each item is one moved/resized element: `sel` is a structural selector rooted at
`.pdp-cmp`, `dx`/`dy` are the move in the element's **local** units, `s` is the
scale multiplier, and `base` is the element's original computed transform (a
`matrix(...)` string, or `"none"`).
...
- Only emit items that actually moved (`dx`/`dy` non-zero or `s` ≠ 1) — the
  editor already filters these, but double-check.
