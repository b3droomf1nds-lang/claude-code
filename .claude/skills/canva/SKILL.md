...
2. Hand the user this preview link and the controls:
   ...
   **Open:** `https://imraiy-tv.myshopify.com/products/voltical-core-5k-10k?preview_theme_id=193289027910&edit=1`
   ...
   - **Align:** snaps small accidental drift (up to 14px, or 40px with Shift+click) on the selected element back to its perfect position — or, if nothing is selected, across every tile, image, icon and text box at once. Horizontal position and size snap back on anything that's only drifted a little; deliberate larger moves are left alone on every axis, including a text box's left/right position (Align used to force text to dead-center unconditionally, which also wiped out intentionally-offset captions — fixed). Vertically, only a tile's own position snaps back to its grid spot — everything a tile contains (images, icons, text) is positioned vertically by its own CSS on purpose, so Align never touches dy for anything but a tile itself. Safe to click any time something looks slightly off after dragging.
